...
/**
 * Data Transfer Object (DTO) carrying the information needed to add a new item. Using a record for
 * immutability and conciseness. Includes basic validation annotations.
 *
...
public record AddItemCommandDto(
    ...
    @NotNull(
            message =
                "Expiration date container cannot be null, " + "use Optional.empty() for no date.")
        Optional<LocalDate> expirationDate) {
  // Canonical constructor Javadoc is implicitly covered by the record Javadoc
  // No explicit canonical constructor needed unless custom validation/normalization required
